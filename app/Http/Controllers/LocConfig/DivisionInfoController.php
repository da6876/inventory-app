...
class DivisionInfoController extends Controller
{
    ...
    {
        return view('LocConfig.division');
    }
    public function store(Request $request){
        try {
            if ($request['id']==""){
                ...
                $id = $request['id'];
                $rowData = DivisionInfo::where('uid', $id)->first();
    ...
    public function destroy($id){
        try {
            $rowData = DivisionInfo::where('uid', $id)->firstOrFail();
            $rowData->update([
                'status' => "Deleted",
                'update_by' => auth()->user()->id,
                'update_date' => $this->getCurrentDateTime()
    ...
    {
        if ($request->ajax()) {
            $query = DivisionInfo::query()->where('status', '!=', 'Deleted');
