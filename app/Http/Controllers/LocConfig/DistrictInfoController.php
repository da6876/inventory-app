...
class DistrictInfoController extends Controller
{
    public function index()
    {
        return view('LocConfig.district');
    }
    public function store(Request $request){
        try {
            if ($request['id']==""){
                ...
                $id = $request['id'];
                $rowData = DistrictInfo::where('uid', $id)->first();
    ...
    public function destroy($id){
        try {
            $rowData = DistrictInfo::where('uid', $id)->firstOrFail();
            $rowData->update([
                'status' => "Deleted",
                'update_by' => auth()->user()->id,
                'update_date' => $this->getCurrentDateTime()
    ...
    {
        $categories = DivisionInfo::where('status', '!=', 'Deleted')->get();
        return response()->json($categories);
    }
    public function getData(Request $request)
    {
        if ($request->ajax()) {
            $query = DistrictInfo::query()->where('status', '!=', 'Deleted');
